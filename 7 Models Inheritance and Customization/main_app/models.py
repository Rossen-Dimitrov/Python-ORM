...
class ZooKeeper(Employee):
    class Expertises(models.TextChoices):
        Mammals = "Mammals",
        Birds = "Birds",
        Reptiles = "Reptiles",
        Others = "Others",
    ...
    def clean(self):
        ...
        if self.specialty not in self.Expertises:
...
class ZooDisplayAnimal(Animal):
    ...
    def display_info(self):
        return (f"Meet {self.name}! It's {self.species} and it's born "
                f"{self.birth_date}. It makes a noise like '{self.sound}'!{self.__extra_info()}")

    def is_endangered(self):
        endangered_list = ["Cross River Gorilla", "Orangutan", "Green Turtle"]
        return True if self.species in endangered_list else False
